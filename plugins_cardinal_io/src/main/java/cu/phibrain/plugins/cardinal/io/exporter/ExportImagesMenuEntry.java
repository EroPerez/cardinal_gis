...
public class ExportImagesMenuEntry extends MenuEntry {
    ...
    private void exportImages(final Context context) {
        try {
            ...
            if (projectName == null) {
                projectName = "cardinalpaparazzi_images_";
            } else {
            ...
            final List<MapObjectImages> imagesList = MapObjectImagesOperations.getInstance().getAll();
                ...
                protected String doBackgroundWork() {
                    try {
                        for (int i = 0; i < imagesList.size(); i++) {
                            MapObjectImages image = imagesList.get(i);
                            try {
