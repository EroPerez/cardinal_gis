...
public class PermissionReadSyncSettings extends AChainedPermissionHelper {

    protected boolean canAskPermission = Build.VERSION.SDK_INT >= Build.VERSION_CODES.M;

    public static int READ_SYNC_SETTINGS_PERMISSION_REQUESTCODE = 91;
                                ...
                                new DialogInterface.OnClickListener() {
                                    ...
                                    public void onClick(DialogInterface dialog, int which) {
                                        activity.requestPermissions(new String[]{
                                                        Manifest.permission.READ_SYNC_SETTINGS},
                                                READ_SYNC_SETTINGS_PERMISSION_REQUESTCODE);
                                    }
                                }
                        );
                        // display the dialog
    ...
    public boolean hasGainedPermission(int requestCode, int[] grantResults) {
        return requestCode == READ_SYNC_SETTINGS_PERMISSION_REQUESTCODE &&
