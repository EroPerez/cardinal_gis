...
public class PermissionWriteSyncSettings extends AChainedPermissionHelper {

    protected boolean canAskPermission = Build.VERSION.SDK_INT >= Build.VERSION_CODES.M;

    public static int WRITE_SYNC_SETTINGS_PERMISSION_REQUESTCODE = 92;
                                ...
                                new DialogInterface.OnClickListener() {
                                    ...
                                    public void onClick(DialogInterface dialog, int which) {
                                        activity.requestPermissions(new String[]{
                                                        Manifest.permission.WRITE_SYNC_SETTINGS},
                                                WRITE_SYNC_SETTINGS_PERMISSION_REQUESTCODE);
                                    }
                                }
                        );
                        // display the dialog
    ...
    public boolean hasGainedPermission(int requestCode, int[] grantResults) {
        return requestCode == WRITE_SYNC_SETTINGS_PERMISSION_REQUESTCODE &&
