...
public class PermissionManageAccount extends AChainedPermissionHelper {

    protected boolean canAskPermission = Build.VERSION.SDK_INT >= Build.VERSION_CODES.M;

    public static int MANAGE_ACCOUNTS_PERMISSION_REQUESTCODE = 87;
        ...
        if (canAskPermission) {
            if (Build.VERSION.SDK_INT >= Build.VERSION_CODES.M) {
                return context.checkSelfPermission(
                        "android.permission.MANAGE_ACCOUNTS") == PackageManager.PERMISSION_GRANTED;
            }
        }
        return true;
    }
                ...
                if (activity.checkSelfPermission(
                    ...
                    if (activity.shouldShowRequestPermissionRationale(
                                ...
                                new DialogInterface.OnClickListener() {
                                    ...
                                    public void onClick(DialogInterface dialog, int which) {
                                        activity.requestPermissions(new String[]{
                                                        "android.permission.MANAGE_ACCOUNTS"},
                                                MANAGE_ACCOUNTS_PERMISSION_REQUESTCODE);
                                    }
                                }
                        );
                        // display the dialog
                        ...
                        activity.requestPermissions(
                                new String[]{"android.permission.MANAGE_ACCOUNTS"},
                                MANAGE_ACCOUNTS_PERMISSION_REQUESTCODE);
                    }
                }
            }
    ...
    public boolean hasGainedPermission(int requestCode, int[] grantResults) {
        return requestCode == MANAGE_ACCOUNTS_PERMISSION_REQUESTCODE &&
