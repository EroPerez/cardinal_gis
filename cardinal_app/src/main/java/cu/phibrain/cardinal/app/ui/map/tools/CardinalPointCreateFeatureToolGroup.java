...
/**
 * The group of tools active when a creating or editing a map object.
 *
 * @author Erodis Pérez Michel (eperezm1986@gmail.com)
 */
public class CardinalPointCreateFeatureToolGroup implements ToolGroup, OnClickListener, OnTouchListener, View.OnLongClickListener {
    ...
    private UserMode userToolActive;
     ...
     */
    public CardinalPointCreateFeatureToolGroup(GPMapView mapView, UserMode mode) {
        this.mapView = mapView;
        this.userToolActive = mode;
    ...
    public void onClick(View v) {
        if (v == addVertexButton) {
            ...
            if (coordinatesList.size() > 0) {
                ...
                IEditableLayer editLayer = EditManager.INSTANCE.getEditLayer();
                try {
                    if (this.userToolActive == UserMode.OBJECT_ADDITION) {
                        for (Geometry geometry : geomsList) {
                            editLayer.addNewFeatureByGeometry(geometry, LibraryConstants.SRID_WGS84_4326);
                        }
                    } else if (this.userToolActive == UserMode.OBJECT_EDITION || this.userToolActive == UserMode.OBJECT_COORD_EDITION) {
                        editLayer.updateFeatureGeometry(null, pointGeometry, LibraryConstants.SRID_WGS84_4326);
                    }
